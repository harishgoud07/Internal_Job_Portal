...
function get_projects_list(){
    $select_pojects_query = $this->db->select()
             ->from('ijp_projects_list');
    ...
    function update_posts($values){
        ...
        $update_post_values['experience'] = 12;$values['experience'];
        $update_post_values['project_id'] = $values['project_id'];
        $update_post_values['date_of_modification'] = new Zend_Db_Expr('now()');
         $this->db->update('ijp_job_posts',$update_post_values,array('post_id =?'=>$values['post_id']));
    }

    function update_post_status($values){
        $this->db->update('ijp_job_posts',array('status'=>$values['status']),array('post_id =?'=>$values['post_id']));
    }
    ...
    function get_requested_posts_count(){
         ...
         $select_post_requests_count_query = $this->db->select()
             ->from('ijp_job_posts',array('requested_posts_count'=>'count(*)'))
             ->where('status = ?','P');
...
$select_post_query = $this->db->select()
          ->from(array('posts'=>'ijp_job_posts'))
          ->join(array('projects'=>'ijp_projects_list'),'posts.project_id = projects.project_id')
